...
async function handleRequest(request, ctx) {
  ...
  try {
    ...
    if (url.pathname === "/favicon.ico" || url.pathname === "/favicon.svg") {
      return new Response(FAVICON_SVG, { headers: { "content-type": "image/svg+xml", "cache-control": "no-store" } });
    }
...
const FAVICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="16" fill="#101827"/><circle cx="32" cy="32" r="20" fill="none" stroke="#c8e1ff" stroke-width="5"/><path d="M24 40V25m0 0c0-5 16-5 16 0v15M24 32h16" fill="none" stroke="#c8e1ff" stroke-width="5" stroke-linecap="round"/></svg>`;
...
function renderPage(wallpaper) {
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="description" content="${title} - Bing 每日壁纸"><link rel="icon" href="/favicon.svg?v=2"><title>${title} · Bing 每日壁纸</title><style>
...
</style></head><body><main class="hero"><footer class="content" style="width:100%;max-width:1100px;padding:10px 16px;border:0;border-radius:10px;background:rgba(5,10,20,.42);box-shadow:none;display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap"><span style="color:#c8e1ff;font-size:.76rem;font-weight:700">${title}</span><span style="color:rgba(255,255,255,.7);font-size:.72rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:min(55vw,560px)">${copyright}</span><span style="color:rgba(255,255,255,.55);font-size:.72rem">${date}</span><a href="https://github.com/lgpay/bing_wallpaper" target="_blank" rel="noopener" title="GitHub 项目" aria-label="GitHub 项目" style="color:#c8e1ff;display:inline-flex;align-items:center"><svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2.5a9.5 9.5 0 0 0-3 18.51c.48.09.65-.21.65-.46v-1.68c-2.65.58-3.21-1.12-3.21-1.12-.43-1.1-1.06-1.39-1.06-1.39-.86-.59.07-.58.07-.58.95.07 1.45.98 1.45.98.85 1.45 2.22 1.03 2.76.79.09-.62.33-1.03.6-1.27-2.12-.24-4.35-1.06-4.35-4.72 0-1.04.37-1.89.98-2.56-.1-.24-.42-1.21.09-2.53 0 0 .8-.26 2.62.98a9.1 9.1 0 0 1 4.77 0c1.82-1.24 2.62-.98 2.62-.98.51 1.32.19 2.29.09 2.53.61.67.98 1.52.98 2.56 0 3.67-2.23 4.48-4.36 4.71.34.3.64.88.64 1.78v2.64c0 .25.17.55.65.46A9.5 9.5 0 0 0 12 2.5Z"/></svg></a><a href="/image" download="bing-wallpaper.jpg" title="下载原图" aria-label="下载原图" style="color:#c8e1ff;display:inline-flex;align-items:center"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3v12"/><path d="m7 10 5 5 5-5"/><path d="M5 21h14"/></svg></a><a href="${source}" target="_blank" rel="noopener" title="查看图片来源" aria-label="查看图片来源" style="color:#c8e1ff;display:inline-flex;align-items:center"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 11v5"/><path d="M12 8h.01"/></svg></a></footer></main></body></html>`;
}
