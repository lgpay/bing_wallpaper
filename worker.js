...
</style></head><body><main class="hero"><section class="content"><p class="eyebrow">Bing · 每日壁纸</p><h1>${title}</h1><p class="copyright">${copyright}</p><div class="meta"><span>📅 ${date}</span><span>🖼️ Bing Wallpaper</span></div><div class="actions"><a class="button" href="/image" download="bing-wallpaper.jpg">下载原图 ↗</a><a class="button secondary" href="${source}" target="_blank" rel="noopener">查看来源</a><a class="button secondary" href="?api=1">图片信息</a></div></section></main></body></html>`;
}
